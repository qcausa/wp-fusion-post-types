...
class WPF_CPT extends WPF_CPT_Integrations_Base {
	...
	private $post_types = [];
	private $synced_post_types = array(); // Property to store post types
	...
	public function init() {
		// Retrieve post types on init and store them in a property.
        add_action( 'init', array( $this, 'retrieve_post_types' ), 20 );
		add_action( 'init', array( $this, 'adjust_wpfusion_init_priority' ), 20 );
		add_action( 'init', array( $this, 'register_dynamic_actions' ), 99 );
		...
		// 	if ( strpos( $tag, 'wpf_configure_settings' ) !== false || strpos( $tag, 'init' ) !== false ) {
		// 		BugFu::log( "Hook fired: {$tag}" );
		// 	}
		// });
        ...
        add_filter( 'wpf_configure_sections', array( $this, 'configure_sections' ), 9999, 2 );
		add_filter( 'wpf_configure_settings', array( $this, 'register_settings' ), 9999, 2 );
		...
		//add_filter( 'wpf_get_setting_tribe_events_fields', array( $this, 'handle_get_tribe_events_fields' ) );
	...
	private function remove_action_and_get_callback( $tag, $class_name, $method_name, $priority = 10 ) {
		global $wp_filter;
		...
		foreach ( $wp_filter[ $tag ]->callbacks[ $priority ] as $unique_id => $callback ) {
			if ( is_array( $callback['function'] ) ) {
				if ( is_object( $callback['function'][0] ) ) {
					$class = get_class( $callback['function'][0] );
					if ( $class === $class_name && $callback['function'][1] === $method_name ) {
						// Remove and return the callback
						unset( $wp_filter[ $tag ]->callbacks[ $priority ][ $unique_id ] );
	...
	public function adjust_wpfusion_init_priority() {
		// Remove the 'init' action added by WP Fusion
		$callback = $this->remove_action_and_get_callback( 'init', 'WP_Fusion_Options', 'init', 10 );

	
		if ( $callback ) {
			// Re-add the action with a higher priority (e.g., 99)
	...
	public function retrieve_post_types() {
        ...
        // BugFu::log('Post types retrieved on init');
        // BugFu::log($this->post_types);
    }
	...
	public function register_dynamic_actions() {
		...
		//BugFu::log($post_types);

		// BugFu::log($post_types);
		$options = get_option('wpf_options');
		
		foreach ($post_types as $post_type) {
			if (isset($options['post_type_sync_' . $post_type->name])){
				//BugFu::log("post_type_sync_" . $post_type->name . " is set");
				if (!empty($options['post_type_sync_' . $post_type->name])) {
					//BugFu::log("post_type_sync_" . $post_type->name . " is not empty");
					$this->synced_post_types[] = $post_type;
			

				// WPF Post Type Fields Table Rendering
				// Pass the post type name as a variable to the function
				// add_action( "show_field_{$post_type->name}-fields", function( $arg1 = null, $arg2 = null ) use ( $post_type ) {
				// 	$this->show_field_postType_fields( $arg1, $arg2, $post_type->name );
				// }, 15, 2 );
				...
				//BugFu::log("wpf_{$post_type->name}_meta_fields set");
	...
	public function register_settings( $settings, $options ) {
		// BugFu::log("register_settings init");
		...
		// 	//BugFu::log("Tribe__Events__Main exists");
		// 	if ( ! isset( $post_types['tribe_events'] ) ) {
		// 		$post_types['tribe_events'] = (object) [
		// 			'name'  => 'tribe_events',
		...
		// 	}
		// } else {
		// 	//BugFu::log("Tribe__Events__Main does not exist");
		// }
		//BugFu::log($this->post_types);
		...
		if ( ! empty( $this->synced_post_types ) ) {
			foreach ( $this->synced_post_types as $post_type ) {
				$settings[ 'postType_' . $post_type->name . '_fields' ] = array( 
					'title'   => __( $post_type->label . ' Fields', 'wp-fusion-lite' ),
					'std'     => array(),
					'type'    => $post_type->name . '-fields',
					'section' => $post_type->name . '-fields',
					'choices' => array(),
				);
			}
		}
	...
	public function show_field_postType_fields( $id, $field) {
		...
		foreach ( wpf_get_option( 'post-fields', array() ) as $key => $data ) {
		...
		foreach ( $field['choices'] as $meta_key => $data ) {
			// BugFu::log($this->options[ $id ][ $meta_key ]);
		...
		echo '<table id="' . $id . '-table" class="table table-hover">';
		...
		foreach ( $field_groups as $group => $group_data ) {
			...
			foreach ( $group_data['fields'] as $user_meta => $data ) {
				...
				wpf_render_postType_field_select( $this->options[ $id ][ $user_meta ]['crm_field'], 'wpf_options', 'postType_post_fields', $user_meta );
				BugFu::log($this->options[ $id ][ $user_meta ]['crm_field']);
	 ...
	 */
	 ...
	 public function prepare_post_meta_fields( $meta_fields ) {
		// BugFu::log("prepare_post_meta_fields init");
	...
	public function prepare_tribe_events_meta_fields( $meta_fields ) {
		// BugFu::log("prepare_tribe_events_meta_fields init");
		// Load the reference of standard WP field names and types.
		include WPF_EC_DIR_PATH . '/includes/wordpress-post-fields.php';
	...
	public function handle_get_tribe_events_fields( $fields ) {
		BugFu::log("handle_get_tribe_events_fields init");
		...
		// Retrieve the setting from the custom option.
		$setting = get_option( 'wpf_tribe_events_fields', array() );
		//BugFu::log($setting);
		return ! empty( $setting ) ? $setting : $value;
	}
